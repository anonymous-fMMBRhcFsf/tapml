...
- (instancetype)init {
  if (self = [super init]) {
    // load chat module
    const PackedFunc* f_chat_create = Registry::Get("tapml.llm_chat_create");
    ICHECK(f_chat_create) << "Cannot find tapml.llm_chat_create";
    llm_chat_ = (*f_chat_create)(static_cast<int>(kDLMetal), 0);
    // load image module
    const PackedFunc* f_image_mod_create = Registry::Get("tapml.llm_image_module_create");
    ICHECK(f_image_mod_create) << "Cannot find tapml.llm_image_module_create";
