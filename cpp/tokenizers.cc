...
Tokenizer Tokenizer::FromPath(const String& _path) {
  ...
  if (std::filesystem::exists(sentencepiece)) {
    LOG(WARNING)
        ...
        << "with TapML again, or see if TapML's huggingface provides this file.";
    return Tokenizer(
...
TVM_REGISTER_GLOBAL("tapml.Tokenizer").set_body_typed([](const String& path) {
  return Tokenizer::FromPath(path);
});

TVM_REGISTER_GLOBAL("tapml.TokenizerEncode")
...
TVM_REGISTER_GLOBAL("tapml.TokenizerDecode")
