 ...
 * \brief The implementation for runtime module of serving engine module in TapML.
 */
#include "engine.h"
 ...
 public:
  ...
  void AddRequest(Request request) final {
    ...
    if (request->input_total_length >= kv_cache_config_->prefill_chunk_size) {
      ...
      // Todo(tapml-team): Use "maximum single sequence length" after impl input chunking.
...
std::unique_ptr<Engine> CreateEnginePacked(TVMArgs args) {
  static const char* kErrorMessage =
      ...
      "2) (string) path to tokenizer configuration files, which in TapML, usually in a model "
      "weights directory; "
      ...
      "2) (string) Model path which includes weights and tapml-chat-config.json; "
...
class EngineModule : public ModuleNode {
 public:
  TVM_MODULE_VTABLE_BEGIN("tapml.serve.engine");
...
TVM_REGISTER_GLOBAL("tapml.serve.create_engine").set_body_typed(EngineModule::Create);
