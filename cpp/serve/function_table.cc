...
ObjectRef FunctionTable::LoadParams(const std::string& model_path, Device device) {
  if (this->use_disco) {
    ...
    if (this->model_metadata_.params.empty()) {
      ...
      auto load_func_name = getenv("TAPML_INTERNAL_PRESHARD_NUM") == nullptr
                                ? "tapml.loader.LoadMultiGPU"
                                : "tapml.loader.LoadMultiGPUPresharded";
...
void FunctionTable::_InitFunctions() {
  ...
  this->nd_copy_embedding_to_offset_func_ = get_global_func("tapml.copy_embedding_to_offset");
