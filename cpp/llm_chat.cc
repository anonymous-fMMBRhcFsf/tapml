    ...
    if (num_shards > 1) {
      ...
      if (Registry::Get(f_create_process_pool) == nullptr) {
        LOG(FATAL) << "Cannot find process launcher `" << f_create_process_pool << "`. "
                   << "Multi-GPU inference depends on TapML Python API to launch process.";
      }
      std::string ccl;
      ...
      this->use_disco = true;
      this->sess = Session::ProcessSession(num_shards, f_create_process_pool, "tapml.cli.worker");
  ...
  ObjectRef LoadParams(const std::string& model_path, Device device, bool use_presharded_weights) {
    if (this->use_disco) {
      ...
      if (this->model_metadata_.params.empty()) {
        ...
        auto load_func_name = use_presharded_weights ? "tapml.loader.LoadMultiGPUPresharded"
                                                     : "tapml.loader.LoadMultiGPU";
...
class LLMChat {
 ...
 public:
      ...
      if (config["conv_template"].is<picojson::object>()) {
        ...
        LOG(WARNING)
            << "Legacy conversation template detected. It will be deprecated in the future. "
               "Please regenerate tapml-chat-config.json with the latest version";
  ...
  void Reload(TVMArgValue reload_lib, String model_path, String app_config_json = "") {
    ...
    picojson::object model_config;
    {
      std::ifstream config_istream((model_path + "/tapml-chat-config.json").c_str());
  ...
  void ResetChat() {
    // TODO(tapml-team): add conversation_.Reset to preserve system prompt
    // and initial message.
  ...
  ObjectRef EmbedStep(std::string inp, bool append_conversation = true,
    ...
    CHECK(ft_.embed_func_.defined())
        << "In order to use the embedding functionality, make sure you "
           "build the model in TAPML with `sep_embed` option on.";
...
class LLMChatModule : public ModuleNode {
 public:
  ...
  const char* type_key() const final { return "tapml.llm_chat"; }
...
TVM_REGISTER_GLOBAL("tapml.get_delta_message").set_body_typed(GetDeltaMessage);
...
// register as a system function that can be queried
TVM_REGISTER_GLOBAL("tapml.llm_chat_create").set_body_typed([](int device_type, int device_id) {
  return CreateChatModule(DLDevice{static_cast<DLDeviceType>(device_type), device_id});
});

TVM_REGISTER_GLOBAL("tapml.random.set_seed").set_body_typed([](int seed) {
  RandomGenerator::GetInstance().SetSeed(seed);
});

// for TapML RUST API: to force the Rust compiler to link the whole translation unit
extern "C" {
