"""
This file specifies how TapML's GPTNeoX parameter maps from other formats, for example HuggingFace
...
def huggingface(model_config: GPTNeoXConfig, quantization: Quantization) -> ExternMapping:
    ...
    for i in range(model_config.num_hidden_layers):
        ...
        for param_name in ["weight", "bias"]:
            tapml_name = f"{qkv_proj}.{param_name}"
    ...
    for tapml_name, tapml_param in named_parameters.items():
        if tapml_name not in mapping.param_map:
            if ".dense_h_to_4h.bias" in tapml_name or ".dense_4h_to_h.bias" in tapml_name:
                param_dtype = model_config.ffn_out_dtype
            else:
                param_dtype = tapml_param.dtype
            mapping.add_mapping(
                tapml_name,
                [tapml_name],
                functools.partial(
