"""
This file specifies how TapML's Phi parameter maps from other formats, for example HuggingFace
...
def huggingface(model_config: PhiConfig, quantization: Quantization) -> ExternMapping:
    ...
    if model_config.model_type == "mixformer-sequential":
        ...
        for tapml_name, _ in named_parameters.items():
            if tapml_name not in mapping.param_map:
                _add(tapml_name, tapml_name)
    return mapping
...
def phi1_huggingface(model_config: Phi1Config, quantization: Quantization) -> ExternMapping:
    ...
    def _concat_add(tapml_name, hf_names):
        mapping.add_mapping(
            tapml_name,
            hf_names,
            functools.partial(
