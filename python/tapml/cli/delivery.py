"""Continuous model delivery for TapML models."""
import argparse
import dataclasses
import json
import shutil
...
from tapml.support.constants import TAPML_TEMP_DIR
from tapml.support.download import git_clone
...
class DeferredScope:
    ...
    def create_temp_dir(self) -> Path:
        ...
        temp_dir = tempfile.mkdtemp(dir=TAPML_TEMP_DIR)
...
def _run_quantization(
    ...
    succeeded = True
    with tempfile.TemporaryDirectory(dir=TAPML_TEMP_DIR) as output_dir:
        log_path = Path(output_dir) / "logs.txt"
        with log_path.open("a", encoding="utf-8") as log_file:
            assert isinstance(model_info.model, Path)
            logger.info("[TapML] Processing in directory: %s", output_dir)
            # Required arguments
            cmd = [
                sys.executable,
                "-m",
    ...
    for task_index, task in enumerate(spec["tasks"], 1):
        with DeferredScope() as deferred:
            ...
            for quantization in spec["default_quantization"] + task.get("quantization", []):
                ...
                repo = spec.get("destination", "{username}/{model_id}-{quantization}-TapML").format(
                    username=username,
...
def main():
    ...
    parser = ArgumentParser("TapML continuous model delivery")
    parser.add_argument(
        "--username",
        type=str,
        required=True,
