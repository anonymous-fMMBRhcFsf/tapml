...
from tapml.interface.compile import (  # pylint: disable=redefined-builtin
    ModelConfigOverride,
    OptimizationFlags,
    compile,
)
...
from tapml.support.auto_config import (
    detect_tapml_chat_config,
    detect_model_type,
    detect_quantization,
)
from tapml.support.auto_target import detect_system_lib_prefix, detect_target_and_host


def main(argv):
    """Parse command line argumennts and call `tapml.compiler.compile`."""
    ...
    parser = ArgumentParser("tapml compile")
    parser.add_argument(
        "model",
        type=detect_tapml_chat_config,
    ...
    parser.add_argument(
        ...
        + " (default: look up tapml-chat-config.json, choices: %(choices)s)",
    )
    parser.add_argument(
        "--model-type",
        type=str,
