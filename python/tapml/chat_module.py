...
from tapml.protocol.conversation_protocol import Conversation
from tapml.support import logging
from tapml.support.auto_device import detect_device
from tapml.support.config import ConfigBase

from . import base as _

if TYPE_CHECKING:
    from tapml.protocol.openai_api_protocol import ChatCompletionMessage
...
class ConvConfig:  # pylint: disable=too-many-instance-attributes
    ...
    This is an attribute of :class:`tapml.ChatConfig`, which can then be passed in to the
    instantiation of a :class:`tapml.ChatModule` instance to override the default
    setting in ``tapml-chat-config.json`` under the model folder. Note that we will
    ...
    The parameters are the same as :class:`tapml.protocol.conversation_protocol.Conversation`
...
class ChatConfig(ConfigBase):  # pylint: disable=too-many-instance-attributes
    ...
    :class:`tapml.ChatModule` instance to override the default setting in
    ``tapml-chat-config.json`` under the model folder.

    Since the configuration is partial, everything will be ``Optional``.

    Note that we will exploit this class to also represent ``tapml-chat-config.json``
    ...
    model_lib : Optional[str]
        ...
        LLaMA-7B weight variants and still use them in prebuilt TapML Chat apps.
    ...
    sliding_window_size : Optional[int]
        ...
        This flag subjects to future refactoring.
    opt : Optional[str]
        Optimization flags. TapML maintains a predefined set of optimization flags,
...
class GenerationConfig(ConfigBase):  # pylint: disable=too-many-instance-attributes
    ...
    of a :class:`tapml.ChatModule` instance to override the default generation
    setting in ``tapml-chat-config.json`` and ``ChatConfig`` under the model folder.
...
def _get_model_path(model: str) -> Tuple[str, str]:
    ...
    We define "valid" as having an ``tapml-chat-config.json`` right under the folder.
    ...
    model_path : str
        ...
        "tapml-chat-config.json"))`` being ``True``.
    chat_file : str
        Essentially ``os.path.join(model_path, "tapml-chat-config.json")``.
    ...
    if model.startswith("HF://"):
        from tapml.support.download import (  # pylint: disable=import-outside-toplevel
            download_tapml_weights,
        )

        logger.info("Downloading model from HuggingFace: %s", model)
        tapml_dir = download_tapml_weights(model)
        cfg_dir = tapml_dir / "tapml-chat-config.json"
        return str(tapml_dir), str(cfg_dir)
    ...
    candidate_paths = [
        ...
        f"dist/{model}/params",  # Default directory after tapml.build_model()
        f"dist/prebuilt/tapml-chat-{model}",  # Also prebuilt workflow, but missed prefix
    ]

    # Look for the first folder that has `tapml-chat-config.json` under it
    for candidate in candidate_paths:
        chat_file = os.path.join(candidate, "tapml-chat-config.json")
        if os.path.isfile(chat_file):
            logger.info("Using model folder: %s", os.path.abspath(candidate))
            logger.info("Using TapML Chat config: %s", os.path.abspath(chat_file))
    ...
    if found_folder:
        # Error 1: there is a folder, but not an tapml model folder (E1)
        raise FileNotFoundError(
            "The model folder provided does not seem to refer to a valid tapml model folder.\n"
            "Specifically, we cannot find `tapml-chat-config.json`, a required file. You should "
            "provide a path that contains the file.\n"
            "According to your input `model`, we looked at folder(s):\n"
            f"{valid_dir_str}"
            "TapML-Chat consumes models that are processed by the TAPML build process.\n"
        )
    ...
    raise FileNotFoundError(
        ...
        "that it contains `tapml-chat-config.json`, among other essential files.\n"
    )
...
def _get_lib_module_path(  # pylint: disable=too-many-arguments
    ...
    config_file_path : str
        The path to ``tapml-chat-config.json``. Used for error message making.
    ...
    for lib_name in candidate_lib_names:
        ...
        candidate_paths.extend(
            [
                ...
                f"dist/{model}/{lib_name}",  # Default directory after tapml.build_model()
...
def _convert_chat_config_to_json_str(
    ...
    chat_config : Optional[ChatConfig]
        User's input. A partial ChatConfig for overriding ``tapml-chat-config.json``.
    conv_template : Optional[str]
...
class ChatModule:  # pylint: disable=too-many-instance-attributes
    ...
    .. code:: python
        ...
        from tapml import ChatModule
        from tapml.callback import StreamToStdout
    ...
    ----------
    model: str
        The model folder after compiling with TAPML build process. The parameter
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        fcreate_chat_mod = tvm.get_global_func("tapml.llm_chat_create")
        ...
        try:
            ...
            from tapml.interface import jit  # pylint: disable=import-outside-toplevel
    ...
    def generate(
        ...
        progress_callback: object
            ...
            chat module. See `tapml/callback.py` for a full list of available callback classes.
        ...
        .. code-block:: python
          ...
          from tapml import ChatModule, GenerationConfig, callback
          cm = ChatModule(xxx)
    ...
    def reset_chat(self, chat_config: Optional[ChatConfig] = None):
        ...
        override the original `tapml-chat-config.json`.

        Parameters
        ----------
        chat_config : Optional[ChatConfig]
            A ``ChatConfig`` instance partially filled. If specified, the chat
            module will reload the `tapml-chat-config.json`, and override it with
