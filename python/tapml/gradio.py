"""Gradio interface for TapML Chat."""
...
def _parse_args():
    args = argparse.ArgumentParser("TapML-Chat Gradio Interface")
    args.add_argument(
        "--artifact-path",
        type=str,
        default="dist",
...
def _get_all_available_models_under_dir(artifact_path: str) -> Dict[str, str]:
    ...
    For each folder, we count it as a valid TapML model folder if either it contains an
    `tapml-chat-config.json` file, or it contains a `params` folder which contains an
    `tapml-chat-config.json` file. We will map the name of a valid folder to its full path to the
    folder containing `tapml-chat-config.json`.
    """
    ...
    for path in glob.glob(os.path.join(search_dir, "*")):
        if os.path.isdir(path):
            ...
            # check if it contains `tapml-chat-config.json`
            if os.path.exists(os.path.join(path, "tapml-chat-config.json")):
                model_dict[model_name] = os.path.abspath(path)
            # check if it contains `params/tapml-chat-config.json`
            elif os.path.exists(os.path.join(path, "params", "tapml-chat-config.json")):
...
class GradioModule:
    r"""The Gradio module for TapML Chat. Different from ChatModule Python API, Gradio module allows
...
def launch_gradio(
    ...
    title = """<h1 align="center">TapML Chat Gradio Interface</h1>"""
    description = (
        """<h3>Welcome to TapML Chat! Pick a model from your local ids to get started.</h3>"""
    )
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            with gr.Column():
                chat_state = gr.State()
                img_list = gr.State()
                chatbot = gr.Chatbot(label="TapML Chat")
