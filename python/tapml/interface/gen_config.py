"""Generator of tapml-chat-config.json and tokenizer configuration."""
...
from tapml.model import Model
from tapml.quantization import Quantization
from tapml.support import convert_tiktoken, logging
from tapml.support.style import bold, green, red
...
class TapmlChatConfig:  # pylint: disable=too-many-instance-attributes
    """Fields in the dumped `tapml-chat-config.json` file."""
...
def gen_config(  # pylint: disable=too-many-locals,too-many-arguments,too-many-branches,too-many-statements
    ...
    """Entrypoint of TapML Chat configuration generation."""
    # Step 1. Initialize `tapml-chat-config.json` using `config.json`
    ...
    tapml_chat_config = TapmlChatConfig(
    ...
    for generation_config_filename in ["generation_config.json", "config.json"]:
        ...
        if generation_config.exists():
            ...
            for key, value in generation_config_json.items():
                if hasattr(tapml_chat_config, key) and getattr(tapml_chat_config, key) is None:
                    setattr(tapml_chat_config, key, value)
    ...
    # 3.1. Copy over the files and populate tapml_chat_config
    for filename in TOKENIZER_FILES:
        file = config.parent / filename
        if file.exists():
            tapml_chat_config.tokenizer_files.append(filename)
    ...
    if tokenizer_model_file.exists() and (not tokenizer_json_file.exists()):
        ...
        try:
            ...
            tapml_chat_config.tokenizer_files.append("tokenizer.json")
    ...
    if (not tokenizer_json_file.exists()) and list(config.parent.glob("*.tiktoken")):
        try:
            ...
            convert_tiktoken.convert_tiktoken(
                str(config.parent), str(output), tapml_chat_config.context_window_size
            )
            tapml_chat_config.tokenizer_files.append("tokenizer.json")
            tapml_chat_config.tokenizer_files.append("vocab.json")
            tapml_chat_config.tokenizer_files.append("merges.txt")
            tapml_chat_config.tokenizer_files.append("special_tokens_map.json")
    ...
    tapml_chat_config.apply_defaults()
    # Step 5. Dump the configuration file to output directory
    with (output / "tapml-chat-config.json").open("w", encoding="utf-8") as out_file:
        json.dump(dataclasses.asdict(tapml_chat_config), out_file, indent=2)
