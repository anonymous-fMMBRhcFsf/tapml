"""Parameter mapping for converting different LLM implementations to TapML."""
import dataclasses
...
class ExternMapping:
    """Mapping from a parameter name in TapML's model definition to its potential source,
    for example, from TapML parameter "model.layers.2.post_attention_layernorm.weight" to PyTorch's
    ...
    param_map : Dict[str, List[str]]
        ...
        in Llama2, the source of TapML parameter "model.layers.0.self_attn.qkv_proj.weight" from
    ...
    map_func : Dict[str, Callable[[np.ndarray, ...], np.ndarray]]
        ...
        parameters into the TapML parameter. For example, for the above example, the function
        would be: `lambda q, k, v: np.concatenate([q, k, v], axis=0)`.

    unused_params : Set[str]
        Parameter names in the source weights that are not used in the TapML model definition.
    """
    ...
    def add_mapping(
        ...
        """Add a mapping from TapML parameters to source parametes as well as a mapping function."""
...
class QuantizeMapping:
    """Mapping from a parameter in TapML's model definition to its eventual names and values after
    ...
    param_map : Dict[str, List[str]]
        ...
        in certain group quantization, the destinations of TapML parameter "qkv_proj.weight` are:

        - "qkv_proj.weight_quantized"
        - "qkv_proj.weight_scale"

    map_func : Dict[str, Callable[NDArray, List[NDArray]]]
        A dictionary that maps the name of a parameter to a function that splits the TapML parameter
        into the destination parameters.

    Notes
    -----
    There are two forms of weight conversion in TapML, one is A) on-the-fly quantization to the
