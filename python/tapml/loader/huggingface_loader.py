...
from tapml.support.preshard import _sharded_param_name
from tapml.support.style import bold
...
class HuggingFaceLoader:  # pylint: disable=too-few-public-methods
    ...
    extern_param_map : ExternMapping
        The parameter mapping from TapML to HuggingFace PyTorch/SafeTensor.
    ...
    quantize_param_map : Optional[QuantizeMapping]
        The quantization mapping from TapML to quantized TapML parameters.
    """
    ...
    def __init__(
        ...
        extern_param_map : ExternMapping
            Maps an TapML parameter to a list of PyTorch/SafeTensor parameters.

        quantize_param_map: Optional[QuantizeMapping]
            The quantization mapping from TapML to quantized TapML parameters, default to None, which
    ...
    def load(
        ...
        """Load the parameters and yield the TapML parameter and its value.
        ...
        ------
        Tuple[str, NDArray]
            The TapML parameter name and its value, quantized if quantization mapping is provided.
        """
        tapml_names = _loading_order(self.extern_param_map, self.torch_to_path)
        for tapml_name in tqdm(tapml_names):
            param = self._load_tapml_param(tapml_name, device=device)
            if preshard_funcs is not None and tapml_name in preshard_funcs:
                sharded_params = preshard_funcs[tapml_name](param)
                for i, sharded_param in enumerate(sharded_params):
                    sharded_name = _sharded_param_name(tapml_name, i)
                    yield from self._load_or_quantize(sharded_name, sharded_param, device)
            else:
                yield from self._load_or_quantize(tapml_name, param, device)
    ...
    def _load_tapml_param(self, tapml_name: str, device: Optional[Device]) -> NDArray:
        torch_names = self.extern_param_map.param_map[tapml_name]
        ...
        with self.stats.timer("map_time_sec"):
            param = self.extern_param_map.map_func[tapml_name](*torch_params)
        if device:
            return as_ndarray(param, device=device)
        return as_ndarray(param)

    def _load_or_quantize(self, tapml_name, param, device: Device):
        if self.quantize_param_map and tapml_name in self.quantize_param_map.param_map:
            with self.stats.timer("quant_time_sec"):
                q_names = self.quantize_param_map.param_map[tapml_name]
                q_params = self.quantize_param_map.map_func[tapml_name](param)
                device.sync()
...
def _loading_order(param_map: ExternMapping, torch_to_path: Dict[str, Path]) -> List[str]:
    ...
    # Step 2. Build a map from torch parameters to TapML parameters
    torch_to_tapml = defaultdict(list)
    for tapml_name, torch_names in param_map.param_map.items():
        for torch_name in torch_names:
            torch_to_tapml[torch_name].append(tapml_name)
    # Step 3. Construct the ordering that ensures file locality
    order = OrderedDict()
    for _, torch_names in path_to_torch.items():
        for torch_name in torch_names:
            for tapml_name in torch_to_tapml[torch_name]:
                if tapml_name not in order:
                    order[tapml_name] = 1
