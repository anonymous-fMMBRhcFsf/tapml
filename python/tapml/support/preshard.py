...
def _update_quantize_map(
    ...
    param_names: List[str] = [tapml_name]

    if tapml_name in quantize_map.param_map:
        # the parameter is quantized
        quantized_params = quantize_map.param_map[tapml_name]
        param_names = quantized_params
        quantize_func = quantize_map.map_func[tapml_name]

        for worker_id in range(tensor_parallel_shards):
            sharded_tapml_name = _sharded_param_name(tapml_name, worker_id)
            quantize_map.param_map[sharded_tapml_name] = [
                _sharded_param_name(param_name, worker_id) for param_name in quantized_params
            ]
            quantize_map.map_func[sharded_tapml_name] = quantize_func
