...
from .constants import TAPML_MULTI_ARCH
from .style import bold, green, red

if TYPE_CHECKING:
    from tapml.compiler.compile import CompileArgs
...
def detect_cuda_arch_list(target: Target) -> List[str]:
    ...
    if TAPML_MULTI_ARCH is not None:
        multi_arch = [x.strip() for x in TAPML_MULTI_ARCH.split(",")]
    else:
...
def _register_cuda_hook(target: Target):
    if TAPML_MULTI_ARCH is None:
        ...
        logger.info(
            ...
            "Example: TAPML_MULTI_ARCH=70,72,75,80,86,87,89,90a",
            bold("TAPML_MULTI_ARCH"),
        )
        multi_arch = None
    else:
        logger.info("%s %s: %s", FOUND, bold("TAPML_MULTI_ARCH"), TAPML_MULTI_ARCH)
        multi_arch = [x.strip() for x in TAPML_MULTI_ARCH.split(",")]
