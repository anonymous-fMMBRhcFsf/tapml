...
from .constants import TAPML_CACHE_DIR, TAPML_TEMP_DIR
...
def download_tapml_weights(  # pylint: disable=too-many-locals
    model_url: str,
    num_processes: int = 4,
    force_redo: bool = False,
) -> Path:
    """Download weights for a model from the HuggingFace Git LFS repo."""
    prefixes, tapml_prefix = ["HF://", "https://huggingface.co/"], ""
    tapml_prefix = next(p for p in prefixes if model_url.startswith(p))
    assert tapml_prefix
    ...
    if model_url.count("/") != 1 + tapml_prefix.count("/") or not model_url.startswith(tapml_prefix):
        raise ValueError(f"Invalid model URL: {model_url}")
    user, repo = model_url[len(tapml_prefix) :].split("/")
    git_dir = TAPML_CACHE_DIR / "model_weights" / user / repo
    try:
        _ensure_directory_not_exist(git_dir, force_redo=force_redo)
    except ValueError:
        logger.info("Weights already downloaded: %s", bold(str(git_dir)))
        return git_dir
    with tempfile.TemporaryDirectory(dir=TAPML_TEMP_DIR) as tmp_dir_prefix:
