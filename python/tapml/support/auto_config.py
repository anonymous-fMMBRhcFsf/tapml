...
if TYPE_CHECKING:
    from tapml.model import Model  # pylint: disable=unused-import
    from tapml.quantization import Quantization  # pylint: disable=unused-import
...
def detect_tapml_chat_config(tapml_chat_config: str) -> Path:
    """Detect and return the path that points to tapml-chat-config.json.
    If `tapml_chat_config` is a directory, it looks for tapml-chat-config.json below it.

    Parameters
    ---------
    tapml_chat_config : str
        The path to `tapml-chat-config.json`, or the directory containing
        `tapml-chat-config.json`.

    Returns
    -------
    tapml_chat_config_json_path : pathlib.Path
        The path points to tapml_chat_config.json.
    """
    # pylint: disable=import-outside-toplevel
    from tapml.model import MODEL_PRESETS

    from .download import download_tapml_weights

    # pylint: enable=import-outside-toplevel

    if tapml_chat_config.startswith("HF://") or tapml_chat_config.startswith("http"):
        tapml_chat_config_path = Path(download_tapml_weights(model_url=tapml_chat_config))
    elif isinstance(tapml_chat_config, str) and tapml_chat_config in MODEL_PRESETS:
        logger.info("%s tapml preset model: %s", FOUND, tapml_chat_config)
        content = MODEL_PRESETS[tapml_chat_config].copy()
        content["model_preset_tag"] = tapml_chat_config
        temp_file = tempfile.NamedTemporaryFile(  # pylint: disable=consider-using-with
            suffix=".json",
            delete=False,
        )
        logger.info("Dumping config to: %s", temp_file.name)
        tapml_chat_config_path = Path(temp_file.name)
        with tapml_chat_config_path.open("w", encoding="utf-8") as tapml_chat_config_file:
            json.dump(content, tapml_chat_config_file, indent=2)
    else:
        tapml_chat_config_path = Path(tapml_chat_config)
    if not tapml_chat_config_path.exists():
        raise ValueError(f"{tapml_chat_config_path} does not exist.")

    if tapml_chat_config_path.is_dir():
        # search tapml-chat-config.json under path
        tapml_chat_config_json_path = tapml_chat_config_path / "tapml-chat-config.json"
        if not tapml_chat_config_json_path.exists():
            raise ValueError(f"Fail to find tapml_chat_config.json under {tapml_chat_config_path}.")
    else:
        tapml_chat_config_json_path = tapml_chat_config_path

    logger.info("%s model configuration: %s", FOUND, tapml_chat_config_json_path)
    return tapml_chat_config_json_path
...
def detect_config(config: str) -> Path:
    ...
    """
    from tapml.model import MODEL_PRESETS  # pylint: disable=import-outside-toplevel
...
def detect_model_type(model_type: str, config: Path) -> "Model":
    ...
    Returns
    -------
    model : tapml.compiler.Model
        The model type.
    """

    from tapml.model import MODELS  # pylint: disable=import-outside-toplevel
...
def detect_quantization(quantization_arg: str, config: Path) -> "Quantization":
    ...
    config : pathlib.Path
        The path to tapml-chat-config.json.

    Returns
    -------
    quantization : tapml.quantization.Quantization
        The model quantization scheme.
    """
    from tapml.quantization import (  # pylint: disable=import-outside-toplevel
        QUANTIZATION,
    )
